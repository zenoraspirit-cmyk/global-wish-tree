...
function handlePayment(type) {
    ...
    const success = confirm(`Redirecting to Payment Gateway... Amount: $${amount}\nClick OK to simulate success.`);
    if (success) {
        if (type === 'wish') {
...
function findWishToFulfill() {
    ...
    if (wish) {
        ...
        alert("Active wish not found for this name.");
    }
}
...
function finalizeFulfillment() {
    if (currentFulfillTarget) {
        ...
        if (idx !== -1) {
            ...
            alert("Payment successful! Your wish is now glowing gold.");
        }
    }
}